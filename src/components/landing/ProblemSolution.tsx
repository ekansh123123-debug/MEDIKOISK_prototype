...
export const ProblemSolution: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-[#080d1a] border-y border-slate-800/80 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          {/* THE PROBLEM CARD */}
          <div className="p-8 bg-gradient-to-b from-rose-950/20 to-slate-900/40 border border-rose-500/20 rounded-3xl flex flex-col justify-between backdrop-blur-xl shadow-xl hover:border-rose-500/30 transition-all">
            <div>
              <div className="flex items-center justify-between mb-5">
                <span className="text-xs font-black text-rose-400 uppercase tracking-widest">
                  Current Practice Reality
                </span>
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-rose-500/10 border border-rose-500/30 text-rose-300">
              ...
              <ul className="space-y-3.5">
                {problems.map((prob, i) => (
                  <li key={i} className="flex items-start gap-3.5 text-sm text-slate-300">
                    <span className="mt-0.5 shrink-0 p-1 rounded-lg bg-rose-500/10 border border-rose-500/20">{prob.icon}</span>
                    <span>{prob.text}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="mt-8 pt-5 border-t border-rose-500/20 text-xs font-medium text-rose-300 flex items-center justify-between">
              <span>Impact: Diagnostic fatigue, patient friction, and delayed care</span>
              <span className="text-rose-400 font-bold">Status Quo</span>
            </div>
          </div>

          {/* THE SOLUTION CARD */}
          <div className="p-8 bg-gradient-to-b from-teal-950/30 to-slate-900/60 border border-teal-500/30 rounded-3xl flex flex-col justify-between backdrop-blur-xl shadow-xl hover:border-teal-500/50 transition-all">
            <div>
              <div className="flex items-center justify-between mb-5">
                <span className="text-xs font-black text-teal-400 uppercase tracking-widest">
                  MEDIKOISK Enterprise Architecture
                </span>
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-teal-500/10 border border-teal-500/30 text-teal-300">
              ...
              <ul className="space-y-3.5">
                {solutions.map((sol, i) => (
                  <li key={i} className="flex items-start gap-3.5 text-sm text-slate-200">
                    <span className="mt-0.5 shrink-0 p-1 rounded-lg bg-teal-500/10 border border-teal-500/20">{sol.icon}</span>
                    <span>{sol.text}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="mt-8 pt-5 border-t border-teal-500/20 text-xs font-medium text-teal-300 flex items-center justify-between">
              <span>Result: 35–45% consultation time reallocated to physician examination</span>
              <span className="text-teal-400 font-bold">Standard of Care</span>
            </div>
          </div>
        </div>
      </div>
