  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-fade-in space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
        ...
        <Badge variant="teal">DYP Hospital & Research Centre</Badge>
      </div>

      {/* Emergency Flash Feed if active */}
      {emergencyHistory.length > 0 && (
        <div className="p-5 bg-rose-50 dark:bg-rose-950/40 border-2 border-rose-500 rounded-3xl space-y-3 animate-pulse">
          ...
          <div className="space-y-2">
            {emergencyHistory.map((em) => (
              <div key={em.id} className="p-3 bg-white dark:bg-slate-900 rounded-xl border border-rose-200 dark:border-rose-800 flex items-center justify-between text-xs">
                <div>
                  <span className="font-bold text-rose-600 mr-2">{em.tokenNumber}</span>
                  <span className="font-semibold text-slate-900 dark:text-white">{em.patientName}</span>
                  <span className="text-slate-500 ml-2">Trigger: {em.matchedRule} ({em.category})</span>
                  <p className="text-slate-600 dark:text-slate-400 italic text-[11px] mt-0.5">"{em.rawSymptom}"</p>
                </div>
                <button
                  onClick={() => alert(`Nurse dispatched to patient ${em.patientName} (Token ${em.tokenNumber}). Destination: ${em.destinationDepartment}`)}
                  className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-lg text-xs transition-colors"
                >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-teal-600 font-semibold">+18% vs yesterday</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-emerald-600 font-semibold">81.2% digital adoption</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-rose-600 font-semibold">Immediate hard-stop</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-purple-600 font-semibold">96.4% verification rate</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-blue-600 font-semibold">ABDM M1 compliant</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-teal-600 font-semibold">Hindi & Marathi voice</span>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <span className="text-[10px] text-emerald-600 font-semibold">NHA Corrigendum 7</span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-bold text-slate-900 dark:text-white">
            Live OPD Queue & Room Assignments
          </h3>
          <span className="text-xs text-slate-500">Auto-refreshing queue feed</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {tokens.map((token) => (
                <tr key={token.tokenNumber} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td className="px-4 py-3 font-mono font-bold text-teal-600">{token.tokenNumber}</td>
                  <td className="px-4 py-3 font-semibold text-slate-900 dark:text-white">{token.patientName}</td>
                  <td className="px-4 py-3 text-slate-500">{token.age} / {token.gender}</td>
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{token.counterRoom}</td>
