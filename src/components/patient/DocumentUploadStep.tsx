    ...
    <div className="max-w-xl mx-auto py-4 px-2 animate-fade-in">
      <div className="glass-card-elevated rounded-3xl p-6 sm:p-8 border border-teal-500/20 shadow-2xl space-y-6">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-800 text-xs">
          <span className="font-bold text-teal-400">Step: Medical Document Intelligence</span>
          <span className="text-slate-400">TrOCR + BioBERT Extraction</span>
        </div>
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Upload outpatient prescription slips or laboratory reports to auto-extract past medications and chronic diagnoses.
          </p>
        </div>

        {/* Processing Animation */}
        {isProcessing ? (
          <div className="p-8 bg-slate-900/80 rounded-3xl border border-teal-500/40 text-center space-y-4 animate-fade-in backdrop-blur-md">
            <div className="w-16 h-16 rounded-2xl bg-teal-500/10 flex items-center justify-center text-teal-400 mx-auto animate-bounce border border-teal-500/20">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs font-mono text-teal-400 capitalize">
              ...
              {[
                ...
                return (
                  <div key={idx} className="flex flex-col items-center gap-1">
                    <div className={`w-full h-1.5 rounded-full ${isPassed ? 'bg-teal-400' : 'bg-slate-800'}`} />
                    <span className={`text-[9px] font-bold ${isPassed ? 'text-teal-400' : 'text-slate-500'}`}>
                      {s.label}
                    </span>
                  </div>
                );
          ...
          <div className="space-y-4">
            ...
            <div 
              ...
              className="p-8 border-2 border-dashed border-teal-500/40 hover:border-teal-400 bg-teal-500/5 hover:bg-teal-500/10 rounded-3xl text-center cursor-pointer transition-all group backdrop-blur-md"
            >
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-teal-500 to-cyan-400 text-slate-950 flex items-center justify-center mx-auto mb-3 shadow-lg group-hover:scale-110 transition-transform">
                <UploadCloud className="w-7 h-7 text-slate-950" />
              </div>
              <h3 className="text-sm font-bold text-white">
                Select Prescription Document or Take Photo
              </h3>
              <p className="text-xs text-slate-400 mt-1">
                PDF, JPG, PNG (Supports handwritten prescriptions)
              </p>
              <div className="mt-3 inline-flex items-center gap-1.5 text-xs text-teal-400 font-semibold">
            ...
            <div className="p-4 bg-slate-900/60 rounded-2xl border border-slate-800 space-y-2">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
                Or Load Hospital Sample Records:
              </span>
              <div className="space-y-2">
                <button
                  onClick={() => handleProcessSampleDoc(0)}
                  className="w-full p-3 bg-slate-900 hover:bg-slate-800 rounded-xl border border-slate-700/80 flex items-center justify-between text-xs text-left transition-colors"
                >
                ...
                >
                  ...
                  <span className="px-2 py-0.5 rounded text-[10px] bg-sky-500/10 text-sky-300 border border-sky-500/20 font-bold">
                    Biochemistry
                  </span>
                </button>
              </div>
